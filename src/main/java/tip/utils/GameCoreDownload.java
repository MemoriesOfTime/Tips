...
public class GameCoreDownload {
        ...
        if (plugin != null) {
            if (!VersionUtils.checkMinimumVersion(plugin, ACTUAL_MINIMUM_GAME_CORE_VERSION)) {
                ...
                if (file != null) {
                    ...
                    Main.getInstance().getLogger().error("删除旧版本失败！请手动删除！");
                }
            }
        }
            ...
            try {
                ...
                download(url, file, (len, fullLength) -> {
                    ...
                    if (d - last.get() > 15) { // 每15%提示一次
                        Main.getInstance().getLogger().info("已下载：" + d + "%");
                        last.set(d);
                    }
                });
            } catch (Exception e) {
                Main.getInstance().getLogger().error("MemoriesOfTime-GameCore依赖下载失败！");
                return checkAndDownload(++retry);
            }

            Main.getInstance().getLogger().info("MemoriesOfTime-GameCore依赖下载成功！");
    ...
    private static class DownloadTask extends RecursiveAction {
        ...
        public DownloadTask(String strUrl, long start, long end, File file, Consumer<Integer> callback) {
            this.strUrl = strUrl;
            this.start = start;
            this.end = end;
            this.file = file;
    ...
    private static class SubDownloadTask extends DownloadTask {
        ...
        public SubDownloadTask(String strUrl, long start, long end, File file, Consumer<Integer> callback) {
