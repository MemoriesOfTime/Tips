...
public class Api {
    ...
    private String getD() {
        try {
            if (Server.getInstance().getPluginManager().getPlugin("PlaceholderAPI") != null) {
                string = PlaceholderAPI.getInstance().translateString(string, player);
            }
            ...
            Map op = Main.getInstance().getConfig().get("变量显示.玩家权限", new HashMap<String, String>() {
                {
                    put("op", "§c[§e管理员§c]§f");
            ...
            Map mode = Main.getInstance().getConfig().get("变量显示.游戏模式", new HashMap<String, String>() {
                {
                    put("0", "生存");
                    put("1", "创造");
                    put("2", "冒险");
            ...
            Map fly = Main.getInstance().getConfig().get("变量显示.飞行", new HashMap<String, String>() {
                {
                    put("0", "飞行开启");
                    put("1", "飞行关闭");
                }
    ...
    private String getB(){
        ...
        String vip = "无插件";
        try {
            title = Main.getInstance().getConfig().getString("变量显示.无称号显示");
        ...
        try {
            vip = Main.getInstance().getConfig().getString("变量显示.无VIP显示");
